...
pub async fn post_create_graphql_mock(
    ...
    let Ok(params) = serde_json::from_str::<CreateGraphQLMockParams>(&body_str) else {
        return Ok(build_error_response(StatusCode::BAD_REQUEST, "Invalid request body"));
    };

    if params.operation_name.is_empty() {
        return Ok(build_error_response(StatusCode::BAD_REQUEST, "Operation name cannot be empty"));
    }

    if params.path.is_empty() {
        return Ok(build_error_response(StatusCode::BAD_REQUEST, "Path cannot be empty"));
    }

    let key = GraphQLMockKey::new(&params.path, &params.operation_name, params.operation_type);
