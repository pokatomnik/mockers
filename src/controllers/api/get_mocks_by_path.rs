...
pub async fn get_all_mocks_by_path(
    ...
    let Some(cache) = response_cache else {
        return Ok(GET_MOCKS_BY_PATH_FAILED.clone());
    };

    let mocks_by_path = cache.get_by_path(&path).await;

    let body = Ok::<Option<HashMap<String, CachedResponse>>, &str>(mocks_by_path)
        .to_protocol()
        .to_string()
        .into();
...
static GET_MOCKS_BY_PATH_FAILED: LazyLock<Response<Full<Bytes>>> = LazyLock::new(|| {
    let body = Err::<&str, String>(MockersErrors::GetMocksByPathFailed.to_string())
        .to_protocol()
        .to_string()
        .into();
